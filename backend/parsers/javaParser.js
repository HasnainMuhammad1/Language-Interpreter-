/**
 * Java Parser - Converts Java code into an Abstract Syntax Tree (AST)
 * This is a simplified parser that handles common Java constructs
 */
...
class JavaParser {
  ...
  parse() {
    while (this.currentLine < this.lines.length) {
      ...
      if (line.startsWith('import ') || line.startsWith('package ')) {
        const stmt = this.parseImport(line);
  ...
  parseClass(line) {
    const match = line.match(/(?:public\s+)?class\s+(\w+)(?:\s+extends\s+(\w+))?/);
    ...
    while (this.currentLine < this.lines.length) {
      ...
      if (trimmed === '}' && this.getIndentLevel(nextLine) <= classIndent) {
        break;
      }
  ...
  isMethodDeclaration(line) {
    // Check for method patterns: modifier* type name(params)
    return /(?:public|private|protected|static)?\s*\w+\s+\w+\s*\([^)]*\)\s*{?/.test(line);
  }
  ...
  parseMethod(line) {
    ...
    // Parse method body
    this.currentLine++;
    const methodIndent = this.getIndentLevel(line);

    while (this.currentLine < this.lines.length) {
      const nextLine = this.lines[this.currentLine];
      const trimmed = nextLine.trim();

      if (trimmed === '}' && this.getIndentLevel(nextLine) <= methodIndent) {
        break;
      }
  ...
  parseFor(line) {
    ...
    let match = line.match(/for\s*\(\s*(\w+)\s+(\w+)\s*:\s*([^)]+)\)/);
    if (match) {
      return {
        type: 'ForLoop',
        iterator: match[2],
        iterable: this.parseExpression(match[3]),
        body: this.parseBlockBody()
      };
    }

    // Traditional for loop: for (init; condition; increment)
    match = line.match(/for\s*\(([^;]+);([^;]+);([^)]+)\)/);
    if (match) {
  ...
  parseBlockBody() {
    const body = [];
    this.currentLine++;
    const blockIndent = this.getIndentLevel(this.lines[this.currentLine - 1]);

    while (this.currentLine < this.lines.length) {
      const nextLine = this.lines[this.currentLine];
      const trimmed = nextLine.trim();

      if (trimmed === '}' && this.getIndentLevel(nextLine) <= blockIndent) {
        break;
      }
...
module.exports = { parseJava, JavaParser };
